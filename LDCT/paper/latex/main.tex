...
$p \le 0.001$ at every dose). Prior invertible-wavelet denoisers cannot observe
...
Fig.~\ref{fig:mech}a), over all seeds whose checkpoints were retained
(S1: $n=1$; S2: $n=3$).

\begin{table}[t]
\centering
\caption{Round-trip error (relative $L_2$) of the trained transforms, on the
final retained checkpoint of each arm. S1 rests on $n=1$ checkpoint, S2 on
$n=3$; this is a property of what was retained, not of the measurement.}
\label{tab:rt}
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
Arm & Wavelet params & S1 ($n$=1) & S2 ($n$=3)\\
\midrule
\texttt{pr} & 24 & \textbf{1.62e-07} & \textbf{1.65e-07}\\
\texttt{fixed} & 0 & 1.64e-07 & 1.64e-07\\
\texttt{unconstrained} & 64 & \textbf{1.49e-01} & \textbf{1.45e-01}\\
\bottomrule
\end{tabular}
\end{table}

Unconstrained learning drives the transform to a state where \textbf{12--16\% of
the signal is lost in a forward--backward pass} (S1 14.9\%; S2 mean 14.5\%,
per-seed 11.9--16.4\%), against $1.6\times10^{-7}$ --- the float32 limit --- for
the two invertible arms: a gap of \textbf{six orders of magnitude}. The network
must then denoise \emph{and} compensate for its own transform.
