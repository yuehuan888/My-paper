...
> 中写的「闭环误差 **99%**」**是错的**；`LDCT投稿路线` 把 **WE-UNet**
> 列为 LDCT 同题材论文也不对（它是**胸片**辐射剂量论文）。历史文档未回改。

> ⚠️ **2026-09-15 二次勘误（重要）**：上一条勘误把 99% 改成了「12.5–14.5%」，
> 但那个数**同样查无出处**——它是一个硬编码常量，而该常量的注释又以论文为出处，
> 构成循环引用。**唯一可溯的 S1 实测是 0.1492，且 n=1**（`roundtrip_checkpoints_w3.json`），
> S2 才是真 n=3（均值 0.1451，逐种子 0.119–0.164）。论文表格当时写 1.25e-01，
> 与**同一份 JSON 画出的 Fig. 2(a)（1.49e-01）当场打架**。现已全部更正为
> **S1 1.49e-01（n=1）/ S2 1.45e-01（n=3）**，口径统一为 **12–16%**。
> 结论不受影响（换用实测值后「六个数量级」仍成立）。详见 `_worklog_2026-09-15.md`。
